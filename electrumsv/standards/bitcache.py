...
def read_bitcache_message(stream: io.BytesIO) -> BitcacheMessage:
    ...
    while True:
        ...
        if len(chunk_id) == 0:
            return BitcacheMessage(tx_bytes, key_data, tsc_proof_bytes, block_hash, block_height,
                block_position, date_added)
        elif len(chunk_id) != 8:
        ...
        if chunk_id == ChunkIds.TX.encode():
            ...
            print("TSC")
            tsc_proof_bytes, block_hash, block_height, block_position = \
...
def read_bitcache_tscproof_chunk(stream: io.BytesIO, chunk_length: int) \
    ...
    block_position = struct.unpack("<Q", stream.read(8))[0]
    block_hash = stream.read(32)
    proof_length = chunk_length - 8 - 8
    if proof_length <= 0:
        raise ValueError("Proof chunk missing proof data")
    proof_bytes = stream.read(proof_length)
    return proof_bytes, block_hash, block_height, block_position
...
def write_bitcache_transaction_message(stream: io.BytesIO, data: BitcacheMessage) -> None:
    ...
    if data.tsc_proof_bytes is not None:
        write_bitcache_tscproof_chunk(stream, data.tsc_proof_bytes, data.block_hash,
            data.block_height, data.block_position)
...
def write_bitcache_tscproof_chunk(stream: io.BytesIO, proof_bytes: bytes, block_hash: bytes,
        block_height: int, block_position: int) -> None:
    stream.write(ChunkIds.TSC_PROOF.encode())
    length_offset = stream.tell()
    stream.write(b"\0" * 8)
    start_offset = stream.tell()
    stream.write(struct.pack("<Q", block_height))
    stream.write(struct.pack("<Q", block_position))
    assert len(block_hash) == 32
    stream.write(block_hash)
