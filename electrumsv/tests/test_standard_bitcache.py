...
def test_read_bitcache_messages() -> None:
    ...
    for filename in sorted(os.listdir(MINING_BITCACHE_PATH)):
        ...
        with open(metadata_path, "r") as f:
            ...
            assert (b"" if data["block_hash"] is None else hex_str_to_hash(data["block_hash"])) == \
                message.block_hash
            assert (data["block_height"] or 0) == message.block_height
            assert (data["block_position"] or 0) == message.block_position
    ...
    for filename in os.listdir(MINING_TEST_PATH):
        ...
        with open(transaction_path, "rb") as f:
            msg = BitcacheMessage(f.read(), [],
                ...
                b"" if data["block_hash"] is None else hex_str_to_hash(data["block_hash"]),
                data["block_height"] or 0, data["block_position"] or 0, None)
